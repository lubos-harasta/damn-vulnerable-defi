...
describe('[Challenge] Puppet', function () {
  ...
  it('Exploit', async function () {
    ...
    const AttackPuppetPoolFactory = await ethers.getContractFactory(
      'AttackPuppetPool',
      attacker
    );
    this.attackerContract = await AttackPuppetPoolFactory.deploy(
      this.lendingPool.address,
      this.token.address,
      this.uniswapExchange.address
    );
    // transfer funds from the attacker's EOA to the attacker's contract
    ...
    // print eth balance
    const attackerEthBalance = (
      await ethers.provider.getBalance(attacker.address)
    ).toString();
    console.log(
      'attackerEthBalance',
      ethers.utils.formatUnits(attackerEthBalance, 'ether')
    );
  });
