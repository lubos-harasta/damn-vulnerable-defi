...
contract EchidnaSelfie {
    ...
    function setWeiAmountForQueueAction(uint256 _amount) external {
        uint256 governanceTokenBalance = token.getTotalSupplyAtLastSnapshot();
        require(
            _amount > governanceTokenBalance / 2,
            "Amount of out the Scope."
        );
        weiAmount = _amount;
    }

    // TODO: create
    // function setPayloadForQueueAction() external {
    //     //
    // }
    ...
    function queueAction() public {
        ...
        token.snapshot();
        // queue the action
        actionId = governance.queueAction(address(pool), payload, weiAmount);
